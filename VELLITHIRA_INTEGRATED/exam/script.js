...
const DISCORD_WEBHOOK_URL = "https://discord.com/api/webhooks/1482332128258560041/KcOYCSgmbJ5Dc8RkF0K8fHASyEbHoWK83G9ZFYpmCTdRSQBRUk6qEsLxQfiCOnksxg1j";
...
async function assignRole() {
    ...
    try {
        const response = await fetch('/api/pass', {
            ...
            body: JSON.stringify({ discordUsername: state.discordUser.username })
        });
...
function sendToDiscord(score) {
    if (!DISCORD_WEBHOOK_URL) return;

    const status = state.failed ? "❌ DISQUALIFIED" : (score >= 80 ? "✅ PASSED" : "⚠️ FAILED");
    const color = state.failed ? 15158332 : (score >= 80 ? 3066993 : 15105570);

    const embed = {
        title: "Whitelist Evaluation Result",
        color: color,
        fields: [
            { name: "Applicant", value: state.applicantName, inline: true },
    ...
    if (state.failed) {
        embed.fields.push({ name: "Reason", value: state.failReason });
    }
